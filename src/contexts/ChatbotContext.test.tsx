...
describe('ChatbotContext - Direct Purchase Flow', () => {
  /**
   * **Feature: chatbot-purchase-flow, Property 24: Direct purchase temporary cart**
   * For any product, clicking the direct purchase button should create a temporary 
   * cart containing only that product and display the CheckoutForm
   * **Validates: Requirements 10.2**
   */
  it('should create temporary cart with single product for direct purchase', () => {
    fc.assert(
      fc.property(
        ...
        fc.record({
          ...
          fragrance: fc.array(fc.string({ minLength: 1, maxLength: 30 }), { minLength: 1, maxLength: 10 }),
          image: fc.string({ minLength: 1, maxLength: 200 }),
          occasion: fc.constantFrom('데이트', '출근', '캐주얼', '파티', '운동', '여행')
        }),
        (product) => {
          const { result } = renderChatbotHook()
          
          // First, add a message with the product so it can be found
          // This simulates the product being recommended by the chatbot
          act(() => {
            ...
            // Manually add message to state by dispatching
            result.current.state.messages.push(productMessage)
          })
          
          // Now start direct purchase checkout
          act(() => {
            result.current.startCheckout('direct', product.id)
          })
          
          const checkoutState = result.current.state.checkoutState
          
          // Verify checkout state was created with correct properties
          if (!checkoutState) {
            return false
          }
          
          // Property 24: Verify temporary cart contains only the single product
          const hasCorrectMode = checkoutState.mode === 'direct'
          const hasSingleItem = checkoutState.items.length === 1
          const hasCorrectProduct = checkoutState.items[0]?.id === product.id
          const hasCorrectStep = checkoutState.step === 'summary'
          
          return hasCorrectMode && hasSingleItem && hasCorrectProduct && hasCorrectStep
        }
      ),
      { numRuns: 100 }
    )
  ...
  it('should clear temporary cart when direct purchase is cancelled', () => {
    fc.assert(
      fc.property(
        fc.record({
          id: fc.string({ minLength: 1, maxLength: 50 }),
        ...
        (product) => {
          ...
          // Verify checkout state exists
          const checkoutStateBefore = result.current.state.checkoutState
          if (!checkoutStateBefore || checkoutStateBefore.mode !== 'direct') {
            return false
          }
          ...
          // Verify checkout state was cleared (temporary cart removed)
          const checkoutStateAfter = result.current.state.checkoutState
          
          return checkoutStateAfter === null
        }
      ),
      { numRuns: 100 }
    )
  ...
  it('should not affect regular cart when using direct purchase', () => {
    fc.assert(
      fc.property(
        fc.record({
          id: fc.string({ minLength: 1, maxLength: 50 }),
        ...
        (product) => {
          ...
          // Verify that direct purchase creates its own temporary items
          // and doesn't use the regular cart
          if (!checkoutState) {
            return false
          }
          
          // The checkout state should have mode='direct' and contain the product
          // This verifies it's a temporary cart, not the regular cart
          return (
            checkoutState.mode === 'direct' &&
            checkoutState.items.length === 1 &&
            checkoutState.items[0].id === product.id
          )
        }
      ),
      { numRuns: 100 }
